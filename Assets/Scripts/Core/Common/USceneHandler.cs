...
namespace Core
{
    public static class USceneHandler
    {
        ...
        public static async Task<ITaskResult> USceneActivate(SceneIndex? sceneIndex)
        {
            ...
            Send($"UScene {sceneIndex} not found. Activation failed!", LogFormat.Warning);

            var uSceneLoadTAskResult = await USceneLoad(sceneIndex);
            if (uSceneLoadTAskResult.Status == false)
                return new TaskResult(false, Send($"UScene {sceneIndex} can not be loaded!", LogFormat.Warning));

            var uSceneActivateTaskResult = await USceneActivate(sceneIndex);
            if (uSceneActivateTaskResult.Status == false)
                return new TaskResult(false, Send($"UScene {sceneIndex} can not be activated!", LogFormat.Warning));
        ...
        {
            await Task.Delay(1);
            return new TaskResult(true, Send($"UScene {sceneIndex} was unloaded... Not implemented!", LogFormat.Warning));
        }

        public static async Task<ITaskResult> USceneReload(SceneIndex? sceneIndex)
        {
            await Task.Delay(1);
            return new TaskResult(true, Send($"UScene {sceneIndex} was eloaded... Not implemented!", LogFormat.Warning));
        }
        ...
        private static Message Send(string text, LogFormat Warning = LogFormat.None) =>
            Messager.Send(m_Debug, "USceneHandler", text, Warning);
