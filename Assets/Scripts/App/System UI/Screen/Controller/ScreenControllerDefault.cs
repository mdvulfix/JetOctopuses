...
namespace App.Screen
{
    ...
    /*
    public class ScreenControllerDefault : AController, IScreenController
    {
        ...
        {
            if (screen == null)
                return new TaskResult(false, Send($"{screen.GetName()} not found!", LogFormat.Warning));

            if (screen.IsLoaded == true)
                return new TaskResult(true, Send($"{screen.GetName()} is already loaded.", LogFormat.Warning));
        ...
        public async Task<ITaskResult> ScreenActivate(IScreen screen, bool animate)
        {
            ...
            if (screen.IsLoaded == false)
                return new TaskResult(false, Send($"{screen.GetName()} is not loaded.", LogFormat.Warning));

            if (screen.IsActivated == true)
                return new TaskResult(true, Send($"{screen.GetName()} is already activated.", LogFormat.Warning));
        ...
        {
            if (screen == null)
                return new TaskResult(false, Send($"{screen.GetType().Name} not found!", LogFormat.Warning));

            if (screen.IsLoaded == false)
                return new TaskResult(false, Send($"{screen.GetName()} is not loaded.", LogFormat.Warning));

            if (screen.IsActivated == false)
                return new TaskResult(true, Send($"{screen.GetName()} is already deactivated.", LogFormat.Warning));
        ...
        public async Task<ITaskResult> ScreenUnload(IScreen screen)
        {
            ...
            if (screen.IsLoaded == false)
                return new TaskResult(false, Send($"{screen.GetName()} is already unloaded.", LogFormat.Warning));
